...
class EinstellungenImportExport(QDialog):
    ...
    def accept(self):
        if self.buttonBox.button(QDialogButtonBox.StandardButton.Ok).text() == "Importieren...":
            ...
            fd.setModal(True)
            fd.setNameFilters(["InsuGDT-Einstellungsdateien (*.ued)"])
            ...
            if fd.exec() == 1:
                ...
                if "Allgemein" in configImport.sections() or "GDT" in configImport.sections() or "Erweiterungen" in configImport.sections():
                    ...
                    try:
                        ...
                        self.done(1)
                        mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von InsuGDT", "Die Einstellungen wurden erfolgreich importiert. Damit diese wirksam werden, muss InsuGDT neu gestartet werden.\nSoll InsuGDT neu gestartet werden?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
                        ...
                        mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von InsuGDT", "Fehler beim Importieren der Einstellungen: " + str(e), QMessageBox.StandardButton.Ok)
                        mb.exec()
                else:
                    mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von InsuGDT", "Die Datei " + datei + " ist keine gültige InsuGDT-Konfigurationsdatei", QMessageBox.StandardButton.Ok)
                    mb.exec()
        else:
            fd = QFileDialog(self)
            ...
            if fd.exec() == 1:
                ...
                dateiname = datetime.datetime.strftime(datetime.datetime.now(), "%Y%m%d%H%M%S") + "_InsuGdtEinstellungen.ued"
                ...
                try:
                    with open(datei, "w") as exportfile:
                        ...
                        mb = QMessageBox(QMessageBox.Icon.Information, "Hinweis von InsuGDT", "Die gewünschten Einstellungen wurden erfolgreich unter dem Namen " + dateiname + " exportiert.", QMessageBox.StandardButton.Ok)
                        mb.exec()
                        self.done(1)
                except Exception as e:
                    mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von InsuGDT", "Fehler beim Exportieren der Einstellungen: " + str(e), QMessageBox.StandardButton.Ok)
                    mb.exec()
